...
namespace RainyMemory {
    ...
    public:
        enum commandType {
            ...
            BUYTICKET, QUERYORDER, REFUNDTICKET, CLEAN, EXIT, ROLLBACK, NONE
        };
    ...
    private:
        ...
        const string commands[17] = {"query_profile", "query_ticket", "buy_ticket", "login", "logout",
                                     "modify_profile", "query_order", "add_user", "add_train", "release_train",
                                     "query_train", "delete_train", "query_transfer", "refund_ticket",
                                     "rollback", "clean", "exit"};
        
        const commandType types[17] = {QUERYPROFILE, QUERYTICKET, BUYTICKET, LOGIN, LOGOUT,
                                       MODIFYPROFILE, QUERYORDER, ADDUSER, ADDTRAIN, RELEASETRAIN,
                                       QUERYTRAIN, DELETETRAIN, QUERYTRANSFER, REFUNDTICKET,
                                       ROLLBACK, CLEAN, EXIT};
        ...
        void reset() {
            ...
            timeStamp = to_int(str.substr(1, str.length() - 2));
            str = ts.nextToken();
            type = getTypeFromString(str);
            while (!ts.empty()) {
                str = ts.nextToken();
                argument[str[1] - 'a'] = ts.nextToken();
                exist[str[1] - 'a'] = true;
            }
        }
